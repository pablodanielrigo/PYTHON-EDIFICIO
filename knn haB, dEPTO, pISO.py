...
# Concatenar Departamento, Habitación y Piso para formar una etiqueta única
...
pipeline = Pipeline([
    ...
    ('classifier', RandomForestClassifier(n_estimators=500, random_state=42, max_depth=20, min_samples_split=5, min_samples_leaf=2))
])

# Entrenar el modelo con validación cruzada para verificar la precisión
cross_val_scores = cross_val_score(pipeline, X_train, y_train, cv=5)
print(f"Precisión promedio de validación cruzada: {np.mean(cross_val_scores):.2f}")
...
mediciones = []
max_mediciones = 25  # Incrementar el número de mediciones para mayor estabilidad
        ...
        if line:
            ...
            if match_data:
                ...
                if len(mediciones) == max_mediciones:
                    ...
                    departamento_habitacion_piso = prediccion[0]
                    departamento, habitacion, piso = departamento_habitacion_piso.split("_")
                    print(f"Predicción: Departamento {departamento}, Habitación {habitacion}, Piso {piso}")
                    ...
                    with open(output_file, 'a', newline='', encoding='latin-1') as csvfile:
                        fieldnames = ['Fecha', 'Departamento', 'Habitacion', 'Piso']
                        writer = pd.DataFrame([[fecha_hora, departamento, habitacion, piso]], columns=fieldnames)
