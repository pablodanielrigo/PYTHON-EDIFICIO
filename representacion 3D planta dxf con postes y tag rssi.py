...
def extract_perimeter(msp, height=10, floors=6):
    perimeter_3d = []
    base_points = []
...
perimeter_3d, poly3d, base_points, floors, height = extract_perimeter(msp, height=20, floors=6)
...
for i, punto in enumerate(postes):
    ax.scatter(*punto, color='blue', s=60, label=f'Poste {i + 1}')  # Etiquetar cada poste
    ax.text(punto[0], punto[1], punto[2] + 3, f'{i + 1}', color='black', fontsize=11)  # Agregar texto



# Graficar la posición del tag
ax.scatter(*posicion, color='red', s=120, label='Posición del tag')

# Añadir la etiqueta "TAG" al círculo rojo
tag_label = "TAG 1"  # Cambiar el número si se requiere
#ax.text(posicion[0], posicion[1], posicion[2], tag_label, color='black', fontsize=12, ha='center')
ax.text(posicion[0], posicion[1], posicion[2] + 3, tag_label, color='black', fontsize=12, ha='center')  # Desplazamiento en Z
# Etiquetas y título
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
