...
# Función para calcular el peso basado en la distancia al TAG
def calcular_peso_distancia(x_tag, y_tag, x_ancla, y_ancla):
    distancia = np.sqrt((x_tag - x_ancla) ** 2 + (y_tag - y_ancla) ** 2)
    if distancia == 0:
        return 1.0  # Evitar división por cero, asignando peso máximo
    return 1 / distancia  # Inversamente proporcional a la distancia
...
# Asegúrate de que los valores de RSSI están en el rango 0-255 sin invertir
        ...
        while True:
            ...
            if line:
                ...
                if match_data:
                    ...
                    rssi_str = match_data.group('rssi')
                    try:
                        rssi = int(rssi_str)
                    except ValueError:
                    ...
                    if len(mediciones) == max_mediciones:
                        ...
                        rssis = []
                        for ancla in anclas:
                            x_ancla, y_ancla = coordenadas_anclas[ancla]
                            x_tag, y_tag = posicion_tag[0], posicion_tag[1]
                            peso = calcular_peso_distancia(x_tag, y_tag, x_ancla, y_ancla)
                            rssi_promedio = np.mean(mediciones_dict[ancla])
                            rssis.append(rssi_promedio * peso)
...
# Dibujar las posiciones de las anclas
for ancla, (x, y) in coordenadas_anclas.items():
    ax.scatter(x, y, 0, color='blue', s=100, label=f'Ancla {ancla}')
    ax.text(x, y, -1, f'A{ancla}', color='black', fontsize=12, ha='center')
