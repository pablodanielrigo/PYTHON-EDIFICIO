...
file_path = r'C:\Users\Pablo\Desktop\Grafrica de Planta\EDIFICIO5.dxf'  # Asegúrate de que esta ruta es correcta
try:
    ...
    print(f"Error: El archivo '{file_path}' no es un archivo DXF válido o está dañado.")
    exit(1)
...
def extract_walls(msp, height_per_floor=2.55, floors=6):
        ...
        if entity.dxftype() == 'LINE':
            ...
            if length > 5:  # Filtrar líneas cortas
                ...
                if 'perimeter' in layer or 'externa' in layer:
                    ...
                    base_points.append((end.x, end.y))
                else:
    ...
    print(f"Extracted {len(perimeter_walls)} perimeter walls and {len(internal_walls)} internal walls.")
    print(f"Total base points after deduplication: {len(base_points)}")

    # Crear caras de los pisos y techo
    poly3d = []
    for i in range(floors + 1):  # Pisos + techo
        z = height_per_floor * i
        face = [(x, y, z) for (x, y) in base_points]
        if len(face) >= 3:
            poly3d.append(face)  # Crear cara del piso o techo
        else:
            print(f"Skipped face at z={z} due to insufficient points: {len(face)} points.")
...
print(f"Posición del tag calculada: {posicion}")
...
for wall in perimeter_walls:
    if isinstance(wall[0], tuple) and isinstance(wall[1], tuple):
        ...
        xs, ys, zs = zip(*wall)
        if len(wall) >= 3:
            ax.add_collection3d(Poly3DCollection([list(wall)], facecolors='red', linewidths=3, edgecolors='red'))
        else:
            print(f"Skipped perimeter wall face due to insufficient points: {len(wall)} points.")
...
for wall in internal_walls:
    if isinstance(wall[0], tuple) and isinstance(wall[1], tuple):
        ...
        xs, ys, zs = zip(*wall)
        if len(wall) >= 3:
            ax.add_collection3d(Poly3DCollection([list(wall)], facecolors='orange', linewidths=1.5, edgecolors='orange', linestyle='--'))
        else:
            print(f"Skipped internal wall face due to insufficient points: {len(wall)} points.")
...
for face in poly3d:
    if len(face) >= 3:
        ...
        print(f"Skipped poly3d face due to insufficient points: {len(face)} points.")
